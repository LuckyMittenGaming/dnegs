(() => {
  ...
  function hasPlayedMobileIntro() {
    try { return window.sessionStorage.getItem(MOBILE_INTRO_KEY) === '1'; }
    catch { return false; }
  }

  function markMobileIntroPlayed() {
    try { window.sessionStorage.setItem(MOBILE_INTRO_KEY, '1'); }
    catch {}
  }
    ...
    if (!$('[data-mobile-intro-stage]')) {
      document.body.insertAdjacentHTML('beforeend', `
        <div class="mobile-intro-stage" data-mobile-intro-stage aria-hidden="true">
          <video class="mobile-intro-video" data-mobile-intro-video src="${mobileIntroVideo}" playsinline webkit-playsinline preload="auto"></video>
          <img class="mobile-intro-image" data-mobile-intro-image src="${mobileIntroImage}" alt="" decoding="async" fetchpriority="high" />
          <button class="mobile-intro-sound" type="button" data-mobile-intro-sound>Tap to play with sound</button>
        </div>
      `);
    }
  }
  ...
  async function tryPlayWithSound(video, stage) {
    try {
      ...
      if (play && typeof play.catch === 'function') await play;
      ...
      return new Promise((resolve) => {
        ...
        const onTap = async () => {
          try {
            ...
            video.currentTime = 0;
            await video.play();
            done(await waitForVideoPlayback(video));
          } catch { done(false); }
        };
  ...
  async function preloadCriticalAssets() {
    ...
    const assetPromise = Promise.allSettled([...criticalImages.map(preloadImage), ...mobileAssets, ...criticalVideos.map(preloadVideoMetadata), fontsReady]);
  ...
  }, MAX_WAIT_MS + MIN_DN_MS + CURTAIN_HOLD_MS + CURTAIN_RAISE_MS + MOBILE_VIDEO_MAX_MS + MOBILE_INTRO_HOLD_MS + 3800);
})();
