(() => {
  ...
  function initWinnerTool() {
    ...
    function fillSelects() { const cards = allCards(); selectEls.forEach((select) => { select.innerHTML = cards.map((c) => `<option value="${c}">${label(c)}</option>`).join(''); }); }
    ...
    function calculate() { const players = [hero, ...opponents]; const knownBoard = board.slice(0, street); const trials = street === 5 ? 1 : opponents.length > 5 ? 1200 : 2200; const wins = Array(players.length).fill(0); for (let t=0;t<trials;t++) { const dead = new Set([...players.flat(), ...knownBoard]); const deck = shuffle(allCards().filter((c)=>!dead.has(c))); const trialBoard = knownBoard.concat(deck.slice(0, 5 - knownBoard.length)); const scores = players.map((hand)=>scoreSeven([...hand, ...trialBoard])); const max = Math.max(...scores); const winners = scores.map((s,i)=>s===max?i:-1).filter((i)=>i>=0); winners.forEach((i)=>wins[i]+=1/winners.length); } const pct = wins.map((w)=>Math.round((w/trials)*1000)/10); renderOdds(pct, trials); }
    ...
    function render() { const visible = visibleBoard(); boardEl.innerHTML = visible.map((c)=>c?cardHtml(c):blankHtml()).join(''); heroCardsEl.innerHTML = hero.map(cardHtml).join(''); const pos = [[50,3],[16,17],[84,17],[8,48],[92,48],[22,75],[78,75],[50,82]]; opponentsEl.innerHTML = opponents.map((hand,i)=>`<div class="opponent-seat" style="left:${pos[i][0]}%;top:${pos[i][1]}%;translate:-50% -50%"><small>Opponent ${i+1}</small><div class="opp-hole">${hand.map(cardHtml).join('')}</div><strong>0%</strong></div>`).join(''); nextBtn.textContent = street === 0 ? 'Reveal Flop' : street === 3 ? 'Reveal Turn' : street === 4 ? 'Reveal River' : 'Re-Deal'; }
    fillSelects(); randomHero(); dealWorld(true);
    selectEls.forEach((s)=>s.addEventListener('change',()=>dealWorld(true))); opponentSelect.addEventListener('change',()=>dealWorld(true)); $('[data-random-hero]', tool).addEventListener('click',()=>dealWorld(false)); $('[data-redeal]', tool).addEventListener('click',()=>dealWorld(false)); nextBtn.addEventListener('click',revealNextStreet);
  }
  ...
  async function init() { initWinnerTool(); initMagneticButtons(); initDepthTilt(); const hasGsap = await ensureGsap(); if (!hasGsap) { initFallbackReveals(); return; } initCursor(window.gsap); initGsapHero(window.gsap); initScrollTheatre(window.gsap, window.ScrollTrigger); initBioCinema(window.gsap, window.ScrollTrigger); initVelocitySkew(window.gsap, window.ScrollTrigger); window.ScrollTrigger?.refresh?.(); }
