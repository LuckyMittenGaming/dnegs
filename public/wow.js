(() => {
  ...
  function initWinnerTool() {
    ...
    function fillSelects() {
      ...
      selectEls.forEach((select) => { select.innerHTML = cards.map((card) => `<option value="${card}">${label(card)}</option>`).join(''); });
    }
    function randomHero() {
      const deck = shuffle(allCards());
      hero = [deck.pop(), deck.pop()];
      selectEls[0].value = hero[0];
      selectEls[1].value = hero[1];
    }
    function syncHeroFromSelects() {
      hero = [selectEls[0].value, selectEls[1].value];
      if (hero[0] === hero[1]) {
        const next = availableDeck([hero[0]])[0];
        hero[1] = next;
        selectEls[1].value = next;
      }
    }
    function dealWorld(keepHero = true) {
      ...
      board = Array.from({ length: 5 }, () => deck.pop());
      street = 0;
      render();
      calculate();
    }
    function revealNextStreet() {
      street = street === 0 ? 3 : street === 3 ? 4 : street === 4 ? 5 : 0;
      if (street === 0) dealWorld(true); else { render(); calculate(); }
    }
    function scoreFive(cards) {
      const rs = cards.map((card) => rankValue[card[0]]).sort((a,b) => b-a);
      const ss = cards.map((card) => card[1]);
      const counts = {};
      ...
      let straight = 0;
      for (let i = 0; i <= unique.length - 5; i++) { const seq = unique.slice(i, i+5); if (seq[0] - seq[4] === 4) { straight = seq[0]; break; } }
      const flush = ss.every((s) => s === ss[0]);
      const pack = (cat, ks) => ks.reduce((n,k) => n * 15 + k, cat);
    ...
    function calculate() {
      const players = [hero, ...opponents];
      const knownBoard = board.slice(0, street);
      const trials = street === 5 ? 1 : opponents.length > 5 ? 1200 : 2200;
      const wins = Array(players.length).fill(0);
      for (let t=0; t<trials; t++) {
        const dead = new Set([...players.flat(), ...knownBoard]);
        const deck = shuffle(allCards().filter((card) => !dead.has(card)));
        const trialBoard = knownBoard.concat(deck.slice(0, 5 - knownBoard.length));
        const scores = players.map((hand) => scoreSeven([...hand, ...trialBoard]));
        const max = Math.max(...scores);
        const winners = scores.map((score, i) => score === max ? i : -1).filter((i) => i >= 0);
        winners.forEach((i) => wins[i] += 1 / winners.length);
      }
      const pct = wins.map((w) => Math.round((w / trials) * 1000) / 10);
      renderOdds(pct, trials);
    }
    ...
    function render() {
      boardEl.innerHTML = visibleBoard().map((card) => card ? cardHtml(card) : blankHtml()).join('');
    ...
    randomHero();
    dealWorld(true);
    selectEls.forEach((select) => select.addEventListener('change', () => dealWorld(true)));
